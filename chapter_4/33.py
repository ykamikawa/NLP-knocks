...
# 確認用の出現順リスト
verbs_test = [morpheme["surface"] for line in neko_lines() for morpheme in line if morpheme["pos"] == "名詞" and morpheme["pos1"] == "サ変接続"]
# 1文ずつ辞書のリストを取得し動詞を抽出
verbs = set(verbs_test)

# 確認のためverbs_testを使って出現順にsort
print(sorted(verbs, key=verbs_test.index)[:50])
#print(verbs_test)
